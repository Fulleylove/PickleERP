...
<a href='load.php?page=FinanceManagementInvoices.php'>Go Back</a><br>
	...
	<!-- 			InvoiceLineID	InvoiceID	Item	Description	UnitPrice	TaxRate	AmountGBP -->
			...
			$sql = "SELECT * FROM `invoicelines` WHERE `InvoiceID` = '$id' ";
		...
		<tr>
			...
			<input type="hidden" name="table_name" value="invoicelines">
			<input type="hidden" name="InvoiceID" value="<?php echo $id ;?>">
			<input type="hidden" name="page_relocate" value="load.php?page=FinanceManagementInvoice.php?id=<?php echo $id;?>">
		...
		<h3>Payments</h3>
			...
			$sql = "SELECT * FROM `payments` WHERE `ReferenceType` = 'invoice' AND `ReferenceID` = '$id'";
		...
		<tr>
			...
			<input type="hidden" name="ReferenceType" value="invoice">
